...
def handle_invite(data, udp_src_addr):
    ...
    if callee in registered_users:
        ...
        # relay_port = relay.get_port()
        # ✅ セッション情報を保存（タイムスタンプ付き）
        # call_sessions[call_id] = {
        #     'from': udp_src_addr,
        #     'to': dst,
        #     'timestamp': now  # ← これがDUPLICATE_SUPPRESS_SECONDS秒以内判定に使われる
        # }        
...
#     例: From: "004" <sip:004@1.2.3.4> → '004'
#     """
#     match = re.search(r'^From:\s*.*?<sip:([^@>]+)@', msg, re.MULTILINE | re.IGNORECASE)
#     return match.group(1) if match else None
...
#     """
#     Viaヘッダーに received と rport を追加する（SIPリクエスト時）。
#     src_addr: 実際にこのメッセージを送信してきたIP/Port (NAT後の外部アドレス)
#     """
#     new_lines = []
#     for line in msg.splitlines():
#         if line.lower().startswith("via:"):
#             # 既に rport がある場合のみ対応（RFC 3581）
#             if "rport" in line:
#                 # rport に数値が入ってない場合に追加
#                 line = re.sub(r"rport(?!\=)", f"rport={src_addr[1]}", line)
#             else:
#                 # rport がなければ追加（;rport=...）
#                 line = line.strip() + f";rport={src_addr[1]}"

#             # received がなければ追加
#             if "received=" not in line:
#                 line = line.strip() + f";received={src_addr[0]}"
...
def rewrite_contact_and_sdp_for_nat(msg: str, src_addr: tuple, call_id: str, rtp_pool) -> str:
    ...
    ip = PROXY_PUBLIC_IP
    rtp_relay, rtcp_relay = rtp_pool.create_session(call_id)
    # relay_port = rtp_relay.get_port()
    # rtcp_port = rtcp_relay.get_port()  # 必要に応じて使用
    ...
    for line in msg.splitlines():
        ...
        if rewrite_contact:
            ...
            #     # 表示名ありでもマッチするようにする
            #     # 例: Contact: "002" <sip:002@192.168.25.14:5064;transport=udp>
            #     match = re.search(r'^(Contact:\s*(?:"[^"]*"\s*)?)<sip:([^@>]+)@[^:>]+(?::(\d+))?(;[^>]*)?>', line, re.IGNORECASE)
            #     if match:
            #         prefix = match.group(1)              # Contact: "002"  もしくは Contact:
            #         username = match.group(2)            # SIPユーザー名（002など）
            #         # match.group(3) = 元のポート（不要）
            #         suffix = match.group(4) or ''        # 末尾の ;transport=udp など
            #         new_contact = f'{prefix}<sip:{username}@{ip}:{SIP_PORT}{suffix}>'
            #         new_lines.append(new_contact)
            #         continue
            # Contact ヘッダー書き換え（display name を含めて維持）
            if stripped.lower().startswith("contact:"):
        ...
        #     # Contactヘッダの書き換え
        #     if stripped.lower().startswith("contact:"):
        #         # 元のhost/portだけ置き換え、その他の属性は残す
        #         match = re.search(r'(<sip:[^@>]+)@([^;>]+)([^>]*)>', line)
        #         if match:
        #             prefix = match.group(1)  # <sip:004
        #             suffix = match.group(3)  # ;transport=udp など
        #             new_contact = f"Contact: {prefix}@{ip}:{SIP_PORT}{suffix}>"
        #             new_lines.append(new_contact)
        #             continue
