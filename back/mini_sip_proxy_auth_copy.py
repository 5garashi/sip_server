...
def clear_session(call_id):
    return  # セッションは即座にはクリアしない。ガーベージコレクションで削除する。
    """指定されたCall-IDのセッションを削除"""
    if call_id in call_sessions:
        del call_sessions[call_id]
        rtp_pool.remove_session(call_id)
        log(f"[INFO] セッション削除: Call-ID={call_id}", "debug")
    else:
        log(f"[WARN] セッション削除失敗: Call-ID={call_id} は存在しません", "debug")
...
def handle_response(data, udp_src_addr):
    ...
    if call_id in call_sessions:
        ...
        if "100 TRYING" in status_line:
            ...
            rtp_pool.remove_session(call_id)
            send_to(data, dst)
            log(f"[送信] 486 Busy Here → {dst}（セッション削除）")
        elif "487 REQUEST TERMINATED" in status_line:
            send_487_terminated(data, dst)
            log(f"[送信] 487 Request Terminated → 発信者 {dst} に転送")
                ...
                if in_sdp:
                    ...
                    #         continue
                        ...
                        if len(parts) >= 2:
                            ...
                            parts[-1] = PROXY_PUBLIC_IP                    # 最後のIPをPUBLIC_IPに
                            new_lines.append(" ".join(parts))
                            continue
...
def handle_misc_method(method, msg, udp_src_addr):
    """
    INFO, MESSAGE, NOTIFY などの未処理SIPメソッドに対して、
    ログ出力および 200 OK を返す。
    """
...
def main_loop():
    while True:
        ...
        log(f"[受信] {method} from {udp_src_addr}")
